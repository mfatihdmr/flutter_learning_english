...
class MatchingColorScreen extends StatefulWidget {
  @override
  MatchingColorScreenState createState() => MatchingColorScreenState();
}

class MatchingColorScreenState extends State<MatchingColorScreen> {
  ...
  Future<void> loadWords() async {
    try {
      ...
      List<dynamic> decodedWords = json.decode(data)['matching_colors'];
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: const Text('Tekrar Oyna', style: TextStyle(color: Colors.white)), // Açık renk
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Renk Eşleştirme Oyunu', style: TextStyle(color: Colors.white)), // Açık renk
        backgroundColor: isDarkMode ? Colors.black : Colors.deepPurpleAccent, // AppBar color
    ...
    : Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: isDarkMode
                ? [Colors.black, Colors.grey[850]!] // Update this with the same gradient or colors used in HomeScreen
                : [Colors.deepPurpleAccent, Colors.purpleAccent], // Light mode background gradient
